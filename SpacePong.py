...
def SpacePong_play():
    ...
    def end_sound():
        winner_effect = mixer.Sound('spacepong_resources/winner_effect.wav')
        winner_effect.play()
    ...
    def toggle_end():
        ...
        if player_2_score < player_1_score:
            bg.bgpic("spacepong_resources/player_1_effect.gif") #displays the winner image
        #urtilizing same measurements and ideas, except a different image is presented
        elif player_1_score < player_2_score:
            bg.bgpic("spacepong_resources/player_2_effect.gif")
        else:
            bg.bgpic("spacepong_resources/game_over_effect.gif")
    ...
    bg.bgpic("spacepong_resources/SpacePong_bg.gif")    

    #Applying Background Music
    pygame.mixer.init() #pygame funtion for mixer initialization, ensures that the music plays
    mixer.music.load('spacepong_resources/background_music.mp3')    #importing the music file from local directory
    ...
    game_quit = False
    running = True
    turtle.TurtleScreen._RUNNING=True           #Solves Turtle Terminator Problem 
    while running:
        if not is_paused:
            ...
            if ball.ycor()>172:
                ...
                bounce_sound = mixer.Sound('spacepong_resources/bounce_effect.wav') #imports the music file from the local directory through the mixer
            ...
            if ball.xcor()>570: #x coordinate pass the x coordinate of BPaddle
                ...
                score_sound = mixer.Sound('spacepong_resources/score_effect.wav')   #imports the music file from the local directory
            ...
            if ball.xcor()<-570:
                ...
                score_sound = mixer.Sound('spacepong_resources/score_effect.wav')
                score_sound.play()
            ...
            if (ball.xcor() > 480 and ball.xcor() < 520) and (ball.ycor() < BPaddle.ycor() + 60 and ball.ycor() > BPaddle.ycor() - 60): 
                ...
                paddle_sound = mixer.Sound('spacepong_resources/paddle_effect.wav') #imports the music file form local directory
            ...
            if (ball.xcor() < -480 and ball.xcor() > -520) and (ball.ycor() < APaddle.ycor() + 60 and ball.ycor() > APaddle.ycor() - 60):
                ball.setx(-480)
                ballxdirection *= -1
                paddle_sound = mixer.Sound('spacepong_resources/paddle_effect.wav')
                paddle_sound.play()  
        if game_quit == True:         
            running = False      
        else:
